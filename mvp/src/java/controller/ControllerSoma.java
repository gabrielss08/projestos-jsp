 ...
 * @author gabri
 */
@WebServlet(name = "ControllerSoma", urlPatterns = {"/ControllerSoma", "/Calcular"})
public class ControllerSoma extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet ControllerSoma</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet ControllerSoma at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if(action.equals("/Calcular")){
            ...
            if(op==1){
                resultado = acao.CalculaValor(modelo);
            }
            else if(op == 2){
                resultado = acao.CalculaValorSub(modelo);
            }
